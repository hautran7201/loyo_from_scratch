...
def plot_image(image, boxes, image_name='my_plot.png'):
...
def save_model(model=None, loss: dict=None, log_name:str=None, save_weight_only:bool=False, overwrite_dir:bool=False):
    ...
    if model:
        if save_weight_only: 
            torch.save(model.state_dict().cpu(), os.path.join(log_name, 'model.pt'))
        else:
            torch.save(model.cpu(), os.path.join(log_name, 'model.pt'))

    # Save loss
    if loss:
        loss = OmegaConf.to_container(loss, resolve=True)
        with open(os.path.join(log_name, 'loss.json'), 'w') as f:
            json.dump(loss, f)
    ...
    # Copy config file
    shutil.copy(config_path, os.path.join(log_name, 'config.yaml'))